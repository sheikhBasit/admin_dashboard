...
async def get_chat_session_details(session_id: str, user: UserInDB = Depends(get_current_user)):
    """Get a single chat session with its full history"""
    try:
        chat_session = await SessionManager.get_chat_session(session_id)
        if not chat_session:
            raise HTTPException(status_code=404, detail="Chat session not found")

        # Assuming an admin can view any chat session.
        # A check for admin role should be in get_current_user or a dependency.

        return chat_session
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Failed to get chat session details: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get chat session details: {str(e)}"
        )
